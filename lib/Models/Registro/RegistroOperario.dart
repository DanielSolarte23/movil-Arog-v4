...
class Formulario {
  ...
  final bool activo;
  final int creadorId;
  final int formularioTipoId; // Este es el ID que viene desde el backend
  ...
  factory Formulario.fromJson(Map<String, dynamic> json) {
    ...
    List<CampoFormulario> camposList =
        (json['campos'] as List<dynamic>)
            .map((campo) => CampoFormulario.fromJson(campo))
            .toList();

    // Ahora extraemos formularioTipoId desde el primer campo si existe, o lo asignamos como null
    int? formularioTipoIdExtraido =
        camposList.isNotEmpty ? camposList[0].formularioTipoId : null;
    ...
    return Formulario(
      ...
      formularioTipoId:
          formularioTipoIdExtraido ??
          json['id'], // Si es null, lo asignamos al id del formulario
    );
  }
} */
